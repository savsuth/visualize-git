...
class RepositoriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "show renders the visualizations for a repository" do
    get repository_url(owner: "savsuth", name: "ai-memory")

    assert_response :success
    assert_match "savsuth/ai-memory", response.body
  ...
  test "show honors the days window param and falls back on invalid values" do
    get repository_url(owner: "savsuth", name: "ai-memory", params: { days: 15 })
    assert_response :success
    assert_match "last 15 days", response.body

    get repository_url(owner: "savsuth", name: "ai-memory", params: { days: 999 })
    assert_response :success
    assert_match "last 42 days", response.body
  end

  test "show 404s for unknown repositories" do
    get repository_url(owner: "savsuth", name: "does-not-exist")
  ...
  test "create adds a repository and enqueues a sync" do
    assert_difference "Repository.count", 1 do
      assert_enqueued_with(job: SyncRepositoryJob) do
        post repositories_url, params: { full_name: "savsuth/easy-ffmpeg" }
      end
    end
  ...
  test "create over turbo stream updates the dashboard in place" do
    ...
    assert_match "savsuth/easy-ffmpeg", response.body
  ...
  test "create rejects duplicates" do
    assert_no_difference "Repository.count" do
      post repositories_url, params: { full_name: "savsuth/ai-memory" }
    end
  ...
  test "destroy removes the repository" do
    assert_difference "Repository.count", -1 do
      delete repository_url(owner: "savsuth", name: "ai-memory")
    end
